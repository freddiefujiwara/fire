...
vi.mock("@/domain/fire", async (importOriginal) => {
  ...
  return {
    ...
    findFireMonthForMedianDepletion: () => ({ recommendedFireMonth: 36, recommendedFireAge: 43, p50TerminalAssets: 10000, successRate: 0.8, iterations: 6, boundaryHit: null, evaluation: {} }),
...
describe("useFireSimulatorViewModel", () => {
  ...
  it("runs monte carlo only when enabled and clears results when disabled", async () => {
    ...
    expect(vm.monteCarloResults.value.terminalDepletionPlan?.recommendedFireAge).toBe(43);
